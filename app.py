...
WECOM_SECRET = os.environ.get("WECOM_SECRET", "你的Secret")
CUSTOMER_SERVICE_USERID = "zhangsan"   # 改为你的客服企业微信账号
...
SYSTEM_PROMPT = "你是一个友好、专业的客服助手，帮助用户解答问题。"

# ---------- 会话管理 ----------
def get_session_history(session_id):
    if session_id not in memory_store:
        memory_store[session_id] = json.dumps([{"role": "system", "content": SYSTEM_PROMPT}])
...
def get_wecom_access_token():
    ...
    try:
        ...
        if resp.get("errcode") == 0:
            wecom_token_cache["token"] = resp["access_token"]
            wecom_token_cache["expire_time"] = now + resp["expires_in"] - 300
            return resp["access_token"]
        else:
            print("获取token失败:", resp)
...
def is_human_request(message):
    keywords = ["转人工", "人工客服", "人工服务", "找人工", "人工", "真人", "找客服"]
...
def chat():
    ...
    if is_human_request(user_message):
        try:
            notify = f"【用户请求人工客服】\n会话ID: {session_id}\n用户消息: {user_message}\n请及时处理。"
            ok = send_to_wecom(CUSTOMER_SERVICE_USERID, notify)
            reply = "已为您转接人工客服，我们的客服人员将在企业微信上处理您的请求，请稍等。" if ok else "转接人工失败，请稍后再试。"
        except Exception as e:
            print("转人工异常:", e)
            reply = "转接人工时发生内部错误。"
        return jsonify({'reply': reply, 'session_id': session_id, 'human_transferred': ok if 'ok' in locals() else False})
    ...
    payload = {"model": "deepseek-chat", "messages": history, "temperature": 0.7, "stream": False}
